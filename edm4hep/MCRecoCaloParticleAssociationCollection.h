...
public:
  MCRecoCaloParticleAssociationCollectionIterator(size_t index,
                                                  const MCRecoCaloParticleAssociationObjPointerContainer* collection) :
      m_index(index),
      m_object(podio::utils::MaybeSharedPtr<MCRecoCaloParticleAssociationObj>{nullptr}),
      m_collection(collection) {
  }

  MCRecoCaloParticleAssociationCollectionIterator(const MCRecoCaloParticleAssociationCollectionIterator&) = delete;
  MCRecoCaloParticleAssociationCollectionIterator&
  operator=(const MCRecoCaloParticleAssociationCollectionIterator&) = delete;
...
public:
  MCRecoCaloParticleAssociationMutableCollectionIterator(
      size_t index, const MCRecoCaloParticleAssociationObjPointerContainer* collection) :
      m_index(index),
      m_object(podio::utils::MaybeSharedPtr<MCRecoCaloParticleAssociationObj>{nullptr}),
      m_collection(collection) {
  }
  ...
  MCRecoCaloParticleAssociationMutableCollectionIterator(
      const MCRecoCaloParticleAssociationMutableCollectionIterator&) = delete;
  MCRecoCaloParticleAssociationMutableCollectionIterator&
  operator=(const MCRecoCaloParticleAssociationMutableCollectionIterator&) = delete;
...
public:
  ...
  MCRecoCaloParticleAssociationCollection* operator->() {
    return (MCRecoCaloParticleAssociationCollection*)this;
  }
  ...
  void push_back(const MutableMCRecoCaloParticleAssociation& object);
  /// Append an object to the (subset) collection
  void push_back(const MCRecoCaloParticleAssociation& object);
  ...
  void setID(uint32_t ID) final {
    m_collectionID = ID;
    if (!m_isSubsetColl) {
      std::for_each(m_storage.entries.begin(), m_storage.entries.end(), [ID](MCRecoCaloParticleAssociationObj* obj) {
