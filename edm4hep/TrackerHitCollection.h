...
public:
  TrackerHitCollectionIterator(size_t index, const TrackerHitObjPointerContainer* collection) :
      m_index(index), m_object(podio::utils::MaybeSharedPtr<TrackerHitObj>{nullptr}), m_collection(collection) {
  }
...
public:
  ...
  TrackerHitCollection* operator->() {
    return (TrackerHitCollection*)this;
  }
  ...
  void push_back(const MutableTrackerHit& object);
  /// Append an object to the (subset) collection
  void push_back(const TrackerHit& object);
  ...
  void setID(uint32_t ID) final {
    m_collectionID = ID;
    if (!m_isSubsetColl) {
      std::for_each(m_storage.entries.begin(), m_storage.entries.end(), [ID](TrackerHitObj* obj) {
