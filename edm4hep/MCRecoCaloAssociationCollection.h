...
public:
  MCRecoCaloAssociationCollectionIterator(size_t index, const MCRecoCaloAssociationObjPointerContainer* collection) :
      m_index(index),
      m_object(podio::utils::MaybeSharedPtr<MCRecoCaloAssociationObj>{nullptr}),
...
public:
  MCRecoCaloAssociationMutableCollectionIterator(size_t index,
                                                 const MCRecoCaloAssociationObjPointerContainer* collection) :
      m_index(index),
      m_object(podio::utils::MaybeSharedPtr<MCRecoCaloAssociationObj>{nullptr}),
      m_collection(collection) {
  }
  ...
  MCRecoCaloAssociationMutableCollectionIterator&
  operator=(const MCRecoCaloAssociationMutableCollectionIterator&) = delete;
...
public:
  ...
  MCRecoCaloAssociationCollection* operator->() {
    return (MCRecoCaloAssociationCollection*)this;
  }
  ...
  void push_back(const MutableMCRecoCaloAssociation& object);
  /// Append an object to the (subset) collection
  void push_back(const MCRecoCaloAssociation& object);
  ...
  void setID(uint32_t ID) final {
    m_collectionID = ID;
    if (!m_isSubsetColl) {
      std::for_each(m_storage.entries.begin(), m_storage.entries.end(), [ID](MCRecoCaloAssociationObj* obj) {
