...
public:
  VertexCollectionIterator(size_t index, const VertexObjPointerContainer* collection) :
      m_index(index), m_object(podio::utils::MaybeSharedPtr<VertexObj>{nullptr}), m_collection(collection) {
  }
...
public:
  ...
  void push_back(const Vertex& object);
  ...
  void setID(uint32_t ID) final {
    m_collectionID = ID;
    if (!m_isSubsetColl) {
      std::for_each(m_storage.entries.begin(), m_storage.entries.end(), [ID](VertexObj* obj) {
