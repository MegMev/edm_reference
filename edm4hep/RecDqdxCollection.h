...
public:
  RecDqdxCollectionIterator(size_t index, const RecDqdxObjPointerContainer* collection) :
      m_index(index), m_object(podio::utils::MaybeSharedPtr<RecDqdxObj>{nullptr}), m_collection(collection) {
  }
...
public:
  RecDqdxMutableCollectionIterator(size_t index, const RecDqdxObjPointerContainer* collection) :
      m_index(index), m_object(podio::utils::MaybeSharedPtr<RecDqdxObj>{nullptr}), m_collection(collection) {
  }
...
public:
  ...
  RecDqdxCollection* operator->() {
    return (RecDqdxCollection*)this;
  }
  ...
  void push_back(const MutableRecDqdx& object);
  /// Append an object to the (subset) collection
  void push_back(const RecDqdx& object);
  ...
  void setID(uint32_t ID) final {
    m_collectionID = ID;
    if (!m_isSubsetColl) {
      std::for_each(m_storage.entries.begin(), m_storage.entries.end(), [ID](RecDqdxObj* obj) {
