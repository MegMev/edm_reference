...
public:
  RawCalorimeterHitCollectionIterator(size_t index, const RawCalorimeterHitObjPointerContainer* collection) :
      m_index(index), m_object(podio::utils::MaybeSharedPtr<RawCalorimeterHitObj>{nullptr}), m_collection(collection) {
  }
...
public:
  ...
  RawCalorimeterHitCollection* operator->() {
    return (RawCalorimeterHitCollection*)this;
  }
  ...
  void push_back(const MutableRawCalorimeterHit& object);
  /// Append an object to the (subset) collection
  void push_back(const RawCalorimeterHit& object);
  ...
  void setID(uint32_t ID) final {
    m_collectionID = ID;
    if (!m_isSubsetColl) {
      std::for_each(m_storage.entries.begin(), m_storage.entries.end(), [ID](RawCalorimeterHitObj* obj) {
