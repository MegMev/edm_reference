...
SimTrackerHitCollection::SimTrackerHitCollection(SimTrackerHitCollectionData&& data, bool isSubsetColl) :
    m_isValid(false),
    m_isPrepared(false),
...
SimTrackerHit SimTrackerHitCollection::operator[](std::size_t index) const {
  return SimTrackerHit(m_storage.entries[index]);
}

SimTrackerHit SimTrackerHitCollection::at(std::size_t index) const {
  return SimTrackerHit(m_storage.entries.at(index));
}

MutableSimTrackerHit SimTrackerHitCollection::operator[](std::size_t index) {
  return MutableSimTrackerHit(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableSimTrackerHit SimTrackerHitCollection::at(std::size_t index) {
  return MutableSimTrackerHit(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t SimTrackerHitCollection::size() const {
  return m_storage.entries.size();
}

std::size_t SimTrackerHitCollection::max_size() const {
  return m_storage.entries.max_size();
}

bool SimTrackerHitCollection::empty() const {
  return m_storage.entries.empty();
}

void SimTrackerHitCollection::setSubsetCollection(bool setSubset) {
...
MutableSimTrackerHit SimTrackerHitCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new SimTrackerHitObj());
...
void SimTrackerHitCollection::prepareForWrite() const {
...
void SimTrackerHitCollection::push_back(const MutableSimTrackerHit& object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(SimTrackerHit(object));
  }
}

void SimTrackerHitCollection::push_back(const SimTrackerHit& object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers SimTrackerHitCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<std::uint64_t> SimTrackerHitCollection::cellID(const size_t nElem) const {
...
std::vector<float> SimTrackerHitCollection::eDep(const size_t nElem) const {
...
std::vector<float> SimTrackerHitCollection::time(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<float> SimTrackerHitCollection::pathLength(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.pathLength);
  }
  return tmp;
}

std::vector<std::int32_t> SimTrackerHitCollection::quality(const size_t nElem) const {
...
std::vector<edm4hep::Vector3d> SimTrackerHitCollection::position(const size_t nElem) const {
...
std::vector<edm4hep::Vector3f> SimTrackerHitCollection::momentum(const size_t nElem) const {
...
size_t SimTrackerHitCollection::getDatamodelRegistryIndex() const {
  return edm4hep::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT SimTrackerHitCollection::getSchemaVersion() const {
...
namespace {
  ...
  podio::CollectionReadBuffers createBuffers(bool isSubset) {
    ...
    readBuffers.type = "edm4hep::SimTrackerHitCollection";
    readBuffers.schemaVersion = edm4hep::meta::schemaVersion;
    readBuffers.data = isSubset ? nullptr : new SimTrackerHitDataContainer;
    ...
    readBuffers.createCollection = [](const podio::CollectionReadBuffers& buffers, bool isSubsetColl) {
      SimTrackerHitCollectionData data(buffers, isSubsetColl);
      return std::make_unique<SimTrackerHitCollection>(std::move(data), isSubsetColl);
    };

    readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
      // We only have any of these buffers if this is not a subset collection
      if (buffers.data) {
        buffers.data = podio::CollectionWriteBuffers::asVector<edm4hep::SimTrackerHitData>(buffers.data);
      }
    };
    ...
    readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
      if (buffers.data) {
        ...
        // we can delete
        delete static_cast<edm4hep::SimTrackerHitDataContainer*>(buffers.data);
      }
  ...
  bool registerCollection() {
    const static auto reg = []() {
      ...
      factory.registerCreationFunc("edm4hep::SimTrackerHitCollection", edm4hep::meta::schemaVersion, createBuffers);
      ...
      for (unsigned int schemaVersion = 1; schemaVersion < edm4hep::meta::schemaVersion + 1; ++schemaVersion) {
        podio::SchemaEvolution::mutInstance().registerEvolutionFunc(
            "edm4hep::SimTrackerHitCollection", schemaVersion, edm4hep::meta::schemaVersion,
...
SimTrackerHit SimTrackerHitCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<SimTrackerHitObj>((*m_collection)[m_index]);
  return m_object;
}
...
MutableSimTrackerHit SimTrackerHitMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<SimTrackerHitObj>((*m_collection)[m_index]);
  return m_object;
}
...
SimTrackerHitMutableCollectionIterator& SimTrackerHitMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
std::ostream& operator<<(std::ostream& o, const SimTrackerHitCollection& v) {
  ...
  o << "          id:      cellID:        eDep:        time:  pathLength:     quality:                 position [ x, "
       "y, z]:                 momentum [ x, y, z]:"
    << '\n';

  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " " << std::setw(12) << el.getCellID() << " "
      << std::setw(12) << el.getEDep() << " " << std::setw(12) << el.getTime() << " " << std::setw(12)
      << el.getPathLength() << " " << std::setw(12) << el.getQuality() << " " << std::setw(12) << el.getPosition()
      << " " << std::setw(12) << el.getMomentum() << " " << std::endl;
...
void SimTrackerHitCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
