...
GeneratorPdfInfoCollectionData::GeneratorPdfInfoCollectionData() : m_data(new GeneratorPdfInfoDataContainer()) {
}

GeneratorPdfInfoCollectionData::GeneratorPdfInfoCollectionData(podio::CollectionReadBuffers buffers,
                                                               bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::GeneratorPdfInfoData>());
  }
...
void GeneratorPdfInfoCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers GeneratorPdfInfoCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void GeneratorPdfInfoCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void GeneratorPdfInfoCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new GeneratorPdfInfoObj({index, collectionID}, data);
...
bool GeneratorPdfInfoCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                   bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::GeneratorPdfInfoObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::GeneratorPdfInfoCollection*>(coll);
...
void GeneratorPdfInfoCollectionData::makeSubsetCollection() {
