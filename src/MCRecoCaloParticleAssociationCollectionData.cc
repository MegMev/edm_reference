...
MCRecoCaloParticleAssociationCollectionData::MCRecoCaloParticleAssociationCollectionData() :
    m_rel_rec(new std::vector<edm4hep::CalorimeterHit>()),
    m_rel_sim(new std::vector<edm4hep::MCParticle>()),
    m_data(new MCRecoCaloParticleAssociationDataContainer()) {
...
MCRecoCaloParticleAssociationCollectionData::MCRecoCaloParticleAssociationCollectionData(
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::MCRecoCaloParticleAssociationData>());
  }
...
void MCRecoCaloParticleAssociationCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoCaloParticleAssociationCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void MCRecoCaloParticleAssociationCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void MCRecoCaloParticleAssociationCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new MCRecoCaloParticleAssociationObj({index, collectionID}, data);
...
bool MCRecoCaloParticleAssociationCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                                bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::MCRecoCaloParticleAssociationObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::MCRecoCaloParticleAssociationCollection*>(coll);
...
void MCRecoCaloParticleAssociationCollectionData::makeSubsetCollection() {
