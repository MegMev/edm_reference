...
MCRecoTrackParticleAssociationCollection::MCRecoTrackParticleAssociationCollection() :
    m_isValid(false),
    m_isPrepared(false),
...
MCRecoTrackParticleAssociationCollection::MCRecoTrackParticleAssociationCollection(
    MCRecoTrackParticleAssociationCollectionData&& data, bool isSubsetColl) :
    m_isValid(false),
    m_isPrepared(false),
...
MCRecoTrackParticleAssociation MCRecoTrackParticleAssociationCollection::operator[](std::size_t index) const {
  return MCRecoTrackParticleAssociation(m_storage.entries[index]);
}

MCRecoTrackParticleAssociation MCRecoTrackParticleAssociationCollection::at(std::size_t index) const {
  return MCRecoTrackParticleAssociation(m_storage.entries.at(index));
}

MutableMCRecoTrackParticleAssociation MCRecoTrackParticleAssociationCollection::operator[](std::size_t index) {
  return MutableMCRecoTrackParticleAssociation(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableMCRecoTrackParticleAssociation MCRecoTrackParticleAssociationCollection::at(std::size_t index) {
  return MutableMCRecoTrackParticleAssociation(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t MCRecoTrackParticleAssociationCollection::size() const {
  return m_storage.entries.size();
}

std::size_t MCRecoTrackParticleAssociationCollection::max_size() const {
  return m_storage.entries.max_size();
}

bool MCRecoTrackParticleAssociationCollection::empty() const {
  return m_storage.entries.empty();
}

void MCRecoTrackParticleAssociationCollection::setSubsetCollection(bool setSubset) {
...
MutableMCRecoTrackParticleAssociation MCRecoTrackParticleAssociationCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new MCRecoTrackParticleAssociationObj());

  obj->id = {int(m_storage.entries.size() - 1), m_collectionID};
  return MutableMCRecoTrackParticleAssociation(podio::utils::MaybeSharedPtr(obj));
}

void MCRecoTrackParticleAssociationCollection::clear() {
  m_storage.clear(m_isSubsetColl);
  m_isPrepared = false;
}

void MCRecoTrackParticleAssociationCollection::prepareForWrite() const {
...
void MCRecoTrackParticleAssociationCollection::prepareAfterRead() {
...
bool MCRecoTrackParticleAssociationCollection::setReferences(const podio::ICollectionProvider* collectionProvider) {
  return m_storage.setReferences(collectionProvider, m_isSubsetColl);
}

void MCRecoTrackParticleAssociationCollection::push_back(const MutableMCRecoTrackParticleAssociation& object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(MCRecoTrackParticleAssociation(object));
  }
}

void MCRecoTrackParticleAssociationCollection::push_back(const MCRecoTrackParticleAssociation& object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoTrackParticleAssociationCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<float> MCRecoTrackParticleAssociationCollection::weight(const size_t nElem) const {
...
size_t MCRecoTrackParticleAssociationCollection::getDatamodelRegistryIndex() const {
  return edm4hep::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT MCRecoTrackParticleAssociationCollection::getSchemaVersion() const {
...
namespace {
  ...
  podio::CollectionReadBuffers createBuffers(bool isSubset) {
    ...
    readBuffers.type = "edm4hep::MCRecoTrackParticleAssociationCollection";
    readBuffers.schemaVersion = edm4hep::meta::schemaVersion;
    readBuffers.data = isSubset ? nullptr : new MCRecoTrackParticleAssociationDataContainer;
    ...
    readBuffers.createCollection = [](const podio::CollectionReadBuffers& buffers, bool isSubsetColl) {
      MCRecoTrackParticleAssociationCollectionData data(buffers, isSubsetColl);
      return std::make_unique<MCRecoTrackParticleAssociationCollection>(std::move(data), isSubsetColl);
    };
    ...
    readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
      ...
      if (buffers.data) {
        buffers.data =
            podio::CollectionWriteBuffers::asVector<edm4hep::MCRecoTrackParticleAssociationData>(buffers.data);
      }
    };
    ...
    readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
      if (buffers.data) {
        ...
        // we can delete
        delete static_cast<edm4hep::MCRecoTrackParticleAssociationDataContainer*>(buffers.data);
      }
  ...
  bool registerCollection() {
    const static auto reg = []() {
      ...
      factory.registerCreationFunc("edm4hep::MCRecoTrackParticleAssociationCollection", edm4hep::meta::schemaVersion,
                                   createBuffers);
      ...
      for (unsigned int schemaVersion = 1; schemaVersion < edm4hep::meta::schemaVersion + 1; ++schemaVersion) {
        podio::SchemaEvolution::mutInstance().registerEvolutionFunc(
            "edm4hep::MCRecoTrackParticleAssociationCollection", schemaVersion, edm4hep::meta::schemaVersion,
...
MCRecoTrackParticleAssociation MCRecoTrackParticleAssociationCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<MCRecoTrackParticleAssociationObj>((*m_collection)[m_index]);
  return m_object;
}
...
MCRecoTrackParticleAssociationCollectionIterator& MCRecoTrackParticleAssociationCollectionIterator::operator++() {
  ++m_index;
  return *this;
}

MutableMCRecoTrackParticleAssociation MCRecoTrackParticleAssociationMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<MCRecoTrackParticleAssociationObj>((*m_collection)[m_index]);
  return m_object;
}
...
void MCRecoTrackParticleAssociationCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
