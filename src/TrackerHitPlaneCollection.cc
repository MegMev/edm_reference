...
TrackerHitPlaneCollection::TrackerHitPlaneCollection(TrackerHitPlaneCollectionData&& data, bool isSubsetColl) :
    m_isValid(false),
    m_isPrepared(false),
...
TrackerHitPlane TrackerHitPlaneCollection::operator[](std::size_t index) const {
  return TrackerHitPlane(m_storage.entries[index]);
}

TrackerHitPlane TrackerHitPlaneCollection::at(std::size_t index) const {
  return TrackerHitPlane(m_storage.entries.at(index));
}

MutableTrackerHitPlane TrackerHitPlaneCollection::operator[](std::size_t index) {
  return MutableTrackerHitPlane(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableTrackerHitPlane TrackerHitPlaneCollection::at(std::size_t index) {
  return MutableTrackerHitPlane(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t TrackerHitPlaneCollection::size() const {
  return m_storage.entries.size();
}

std::size_t TrackerHitPlaneCollection::max_size() const {
  return m_storage.entries.max_size();
}

bool TrackerHitPlaneCollection::empty() const {
  return m_storage.entries.empty();
}

void TrackerHitPlaneCollection::setSubsetCollection(bool setSubset) {
...
MutableTrackerHitPlane TrackerHitPlaneCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new TrackerHitPlaneObj());
...
void TrackerHitPlaneCollection::prepareForWrite() const {
...
void TrackerHitPlaneCollection::prepareAfterRead() {
...
bool TrackerHitPlaneCollection::setReferences(const podio::ICollectionProvider* collectionProvider) {
  return m_storage.setReferences(collectionProvider, m_isSubsetColl);
}

void TrackerHitPlaneCollection::push_back(const MutableTrackerHitPlane& object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(TrackerHitPlane(object));
  }
}

void TrackerHitPlaneCollection::push_back(const TrackerHitPlane& object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers TrackerHitPlaneCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<std::uint64_t> TrackerHitPlaneCollection::cellID(const size_t nElem) const {
...
std::vector<std::int32_t> TrackerHitPlaneCollection::type(const size_t nElem) const {
...
std::vector<std::int32_t> TrackerHitPlaneCollection::quality(const size_t nElem) const {
  std::vector<std::int32_t> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<float> TrackerHitPlaneCollection::time(const size_t nElem) const {
...
std::vector<float> TrackerHitPlaneCollection::eDep(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<float> TrackerHitPlaneCollection::eDepError(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<edm4hep::Vector2f> TrackerHitPlaneCollection::u(const size_t nElem) const {
  std::vector<edm4hep::Vector2f> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.u);
  }
  return tmp;
}

std::vector<edm4hep::Vector2f> TrackerHitPlaneCollection::v(const size_t nElem) const {
  std::vector<edm4hep::Vector2f> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.v);
  }
  return tmp;
}

std::vector<float> TrackerHitPlaneCollection::du(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.du);
  }
  return tmp;
}

std::vector<float> TrackerHitPlaneCollection::dv(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
    tmp.emplace_back(m_storage.entries[i]->data.dv);
  }
  return tmp;
}

std::vector<edm4hep::Vector3d> TrackerHitPlaneCollection::position(const size_t nElem) const {
...
std::vector<edm4hep::CovMatrix3f> TrackerHitPlaneCollection::covMatrix(const size_t nElem) const {
...
size_t TrackerHitPlaneCollection::getDatamodelRegistryIndex() const {
  return edm4hep::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT TrackerHitPlaneCollection::getSchemaVersion() const {
...
namespace {
  ...
  podio::CollectionReadBuffers createBuffers(bool isSubset) {
    ...
    readBuffers.type = "edm4hep::TrackerHitPlaneCollection";
    readBuffers.schemaVersion = edm4hep::meta::schemaVersion;
    readBuffers.data = isSubset ? nullptr : new TrackerHitPlaneDataContainer;
    ...
    readBuffers.createCollection = [](const podio::CollectionReadBuffers& buffers, bool isSubsetColl) {
      TrackerHitPlaneCollectionData data(buffers, isSubsetColl);
      return std::make_unique<TrackerHitPlaneCollection>(std::move(data), isSubsetColl);
    };

    readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
      // We only have any of these buffers if this is not a subset collection
      if (buffers.data) {
        buffers.data = podio::CollectionWriteBuffers::asVector<edm4hep::TrackerHitPlaneData>(buffers.data);
      }
    };
    ...
    readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
      if (buffers.data) {
        ...
        // we can delete
        delete static_cast<edm4hep::TrackerHitPlaneDataContainer*>(buffers.data);
      }
  ...
  bool registerCollection() {
    const static auto reg = []() {
      ...
      factory.registerCreationFunc("edm4hep::TrackerHitPlaneCollection", edm4hep::meta::schemaVersion, createBuffers);
      ...
      for (unsigned int schemaVersion = 1; schemaVersion < edm4hep::meta::schemaVersion + 1; ++schemaVersion) {
        podio::SchemaEvolution::mutInstance().registerEvolutionFunc(
            "edm4hep::TrackerHitPlaneCollection", schemaVersion, edm4hep::meta::schemaVersion,
...
TrackerHitPlane TrackerHitPlaneCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<TrackerHitPlaneObj>((*m_collection)[m_index]);
  return m_object;
}
...
TrackerHitPlaneCollectionIterator& TrackerHitPlaneCollectionIterator::operator++() {
  ++m_index;
  return *this;
}

MutableTrackerHitPlane TrackerHitPlaneMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<TrackerHitPlaneObj>((*m_collection)[m_index]);
  return m_object;
}
...
TrackerHitPlaneMutableCollectionIterator& TrackerHitPlaneMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
std::ostream& operator<<(std::ostream& o, const TrackerHitPlaneCollection& v) {
  ...
  o << "          id:      cellID:        type:     quality:        time:        eDep:   eDepError:               u [ "
       "a, b]:               v [ a, b]:          du:          dv:                 position [ x, y, z]:covMatrix [ "
       "values]:"
    << '\n';
  ...
  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " " << std::setw(12) << el.getCellID() << " "
      ...
      << std::setw(12) << el.getU() << " " << std::setw(12) << el.getV() << " " << std::setw(12) << el.getDu() << " "
      << std::setw(12) << el.getDv() << " " << std::setw(12) << el.getPosition() << " " << std::setw(12)
      << el.getCovMatrix() << " " << std::endl;
  }
...
void TrackerHitPlaneCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
