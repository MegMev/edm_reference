...
TrackerHitPlaneCollectionData::TrackerHitPlaneCollectionData() : m_data(new TrackerHitPlaneDataContainer()) {
}

TrackerHitPlaneCollectionData::TrackerHitPlaneCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::TrackerHitPlaneData>());
  }
...
void TrackerHitPlaneCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers TrackerHitPlaneCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void TrackerHitPlaneCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void TrackerHitPlaneCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new TrackerHitPlaneObj({index, collectionID}, data);
...
bool TrackerHitPlaneCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                  bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::TrackerHitPlaneObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::TrackerHitPlaneCollection*>(coll);
...
void TrackerHitPlaneCollectionData::makeSubsetCollection() {
