...
MCRecoTrackerAssociationCollection::MCRecoTrackerAssociationCollection(MCRecoTrackerAssociationCollectionData&& data,
                                                                       bool isSubsetColl) :
    m_isValid(false),
    m_isPrepared(false),
...
MCRecoTrackerAssociation MCRecoTrackerAssociationCollection::operator[](std::size_t index) const {
  return MCRecoTrackerAssociation(m_storage.entries[index]);
}

MCRecoTrackerAssociation MCRecoTrackerAssociationCollection::at(std::size_t index) const {
  return MCRecoTrackerAssociation(m_storage.entries.at(index));
}

MutableMCRecoTrackerAssociation MCRecoTrackerAssociationCollection::operator[](std::size_t index) {
  return MutableMCRecoTrackerAssociation(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableMCRecoTrackerAssociation MCRecoTrackerAssociationCollection::at(std::size_t index) {
  return MutableMCRecoTrackerAssociation(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t MCRecoTrackerAssociationCollection::size() const {
  return m_storage.entries.size();
}

std::size_t MCRecoTrackerAssociationCollection::max_size() const {
  return m_storage.entries.max_size();
}

bool MCRecoTrackerAssociationCollection::empty() const {
  return m_storage.entries.empty();
}

void MCRecoTrackerAssociationCollection::setSubsetCollection(bool setSubset) {
...
MutableMCRecoTrackerAssociation MCRecoTrackerAssociationCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new MCRecoTrackerAssociationObj());
...
void MCRecoTrackerAssociationCollection::prepareForWrite() const {
...
void MCRecoTrackerAssociationCollection::push_back(const MutableMCRecoTrackerAssociation& object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(MCRecoTrackerAssociation(object));
  }
}

void MCRecoTrackerAssociationCollection::push_back(const MCRecoTrackerAssociation& object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoTrackerAssociationCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<float> MCRecoTrackerAssociationCollection::weight(const size_t nElem) const {
...
size_t MCRecoTrackerAssociationCollection::getDatamodelRegistryIndex() const {
  return edm4hep::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT MCRecoTrackerAssociationCollection::getSchemaVersion() const {
...
namespace {
  ...
  podio::CollectionReadBuffers createBuffers(bool isSubset) {
    ...
    readBuffers.type = "edm4hep::MCRecoTrackerAssociationCollection";
    readBuffers.schemaVersion = edm4hep::meta::schemaVersion;
    readBuffers.data = isSubset ? nullptr : new MCRecoTrackerAssociationDataContainer;
    ...
    readBuffers.createCollection = [](const podio::CollectionReadBuffers& buffers, bool isSubsetColl) {
      MCRecoTrackerAssociationCollectionData data(buffers, isSubsetColl);
      return std::make_unique<MCRecoTrackerAssociationCollection>(std::move(data), isSubsetColl);
    };

    readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
      // We only have any of these buffers if this is not a subset collection
      if (buffers.data) {
        buffers.data = podio::CollectionWriteBuffers::asVector<edm4hep::MCRecoTrackerAssociationData>(buffers.data);
      }
    };
    ...
    readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
      if (buffers.data) {
        ...
        // we can delete
        delete static_cast<edm4hep::MCRecoTrackerAssociationDataContainer*>(buffers.data);
      }
  ...
  bool registerCollection() {
    const static auto reg = []() {
      ...
      factory.registerCreationFunc("edm4hep::MCRecoTrackerAssociationCollection", edm4hep::meta::schemaVersion,
                                   createBuffers);
      ...
      for (unsigned int schemaVersion = 1; schemaVersion < edm4hep::meta::schemaVersion + 1; ++schemaVersion) {
        podio::SchemaEvolution::mutInstance().registerEvolutionFunc(
            "edm4hep::MCRecoTrackerAssociationCollection", schemaVersion, edm4hep::meta::schemaVersion,
...
MCRecoTrackerAssociation MCRecoTrackerAssociationCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<MCRecoTrackerAssociationObj>((*m_collection)[m_index]);
  return m_object;
}
...
MutableMCRecoTrackerAssociation MCRecoTrackerAssociationMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<MCRecoTrackerAssociationObj>((*m_collection)[m_index]);
  return m_object;
}
...
MCRecoTrackerAssociationMutableCollectionIterator& MCRecoTrackerAssociationMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
void MCRecoTrackerAssociationCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
