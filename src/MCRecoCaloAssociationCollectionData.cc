...
MCRecoCaloAssociationCollectionData::MCRecoCaloAssociationCollectionData() :
    m_rel_rec(new std::vector<edm4hep::CalorimeterHit>()),
    m_rel_sim(new std::vector<edm4hep::SimCalorimeterHit>()),
    m_data(new MCRecoCaloAssociationDataContainer()) {
  m_refCollections.emplace_back(std::make_unique<std::vector<podio::ObjectID>>());
  m_refCollections.emplace_back(std::make_unique<std::vector<podio::ObjectID>>());
}

MCRecoCaloAssociationCollectionData::MCRecoCaloAssociationCollectionData(podio::CollectionReadBuffers buffers,
                                                                         bool isSubsetColl) :
    m_rel_rec(new std::vector<edm4hep::CalorimeterHit>()),
    m_rel_sim(new std::vector<edm4hep::SimCalorimeterHit>()),
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::MCRecoCaloAssociationData>());
  }
...
void MCRecoCaloAssociationCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoCaloAssociationCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void MCRecoCaloAssociationCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void MCRecoCaloAssociationCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new MCRecoCaloAssociationObj({index, collectionID}, data);
...
bool MCRecoCaloAssociationCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                        bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::MCRecoCaloAssociationObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::MCRecoCaloAssociationCollection*>(coll);
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      ...
      entries[i]->m_rec = new edm4hep::CalorimeterHit((*tmp_coll)[id.index]);
    } else {
      entries[i]->m_rec = nullptr;
    }
  }
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      ...
      entries[i]->m_sim = new edm4hep::SimCalorimeterHit((*tmp_coll)[id.index]);
    } else {
      entries[i]->m_sim = nullptr;
    }
  }
...
void MCRecoCaloAssociationCollectionData::makeSubsetCollection() {
