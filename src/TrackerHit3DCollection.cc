...
TrackerHit3DCollection::TrackerHit3DCollection() :
    m_isValid(false),
    m_isPrepared(false),
...
TrackerHit3DCollection::TrackerHit3DCollection(TrackerHit3DCollectionData&& data, bool isSubsetColl) :
    m_isValid(false),
    m_isPrepared(false),
...
TrackerHit3D TrackerHit3DCollection::operator[](std::size_t index) const {
  return TrackerHit3D(m_storage.entries[index]);
}

TrackerHit3D TrackerHit3DCollection::at(std::size_t index) const {
  return TrackerHit3D(m_storage.entries.at(index));
}

MutableTrackerHit3D TrackerHit3DCollection::operator[](std::size_t index) {
  return MutableTrackerHit3D(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableTrackerHit3D TrackerHit3DCollection::at(std::size_t index) {
  return MutableTrackerHit3D(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t TrackerHit3DCollection::size() const {
  return m_storage.entries.size();
}

std::size_t TrackerHit3DCollection::max_size() const {
  return m_storage.entries.max_size();
}

bool TrackerHit3DCollection::empty() const {
  return m_storage.entries.empty();
}

void TrackerHit3DCollection::setSubsetCollection(bool setSubset) {
...
MutableTrackerHit3D TrackerHit3DCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new TrackerHit3DObj());

  obj->id = {int(m_storage.entries.size() - 1), m_collectionID};
  return MutableTrackerHit3D(podio::utils::MaybeSharedPtr(obj));
}

void TrackerHit3DCollection::clear() {
  m_storage.clear(m_isSubsetColl);
  m_isPrepared = false;
}

void TrackerHit3DCollection::prepareForWrite() const {
...
void TrackerHit3DCollection::prepareAfterRead() {
...
bool TrackerHit3DCollection::setReferences(const podio::ICollectionProvider* collectionProvider) {
  return m_storage.setReferences(collectionProvider, m_isSubsetColl);
}

void TrackerHit3DCollection::push_back(const MutableTrackerHit3D& object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(TrackerHit3D(object));
  }
}

void TrackerHit3DCollection::push_back(const TrackerHit3D& object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers TrackerHit3DCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<std::uint64_t> TrackerHit3DCollection::cellID(const size_t nElem) const {
...
std::vector<std::int32_t> TrackerHit3DCollection::type(const size_t nElem) const {
...
std::vector<std::int32_t> TrackerHit3DCollection::quality(const size_t nElem) const {
  std::vector<std::int32_t> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<float> TrackerHit3DCollection::time(const size_t nElem) const {
...
std::vector<float> TrackerHit3DCollection::eDep(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<float> TrackerHit3DCollection::eDepError(const size_t nElem) const {
  std::vector<float> tmp;
  const auto valid_size = nElem != 0 ? std::min(nElem, m_storage.entries.size()) : m_storage.entries.size();
  tmp.reserve(valid_size);
  for (size_t i = 0; i < valid_size; ++i) {
...
std::vector<edm4hep::Vector3d> TrackerHit3DCollection::position(const size_t nElem) const {
...
std::vector<edm4hep::CovMatrix3f> TrackerHit3DCollection::covMatrix(const size_t nElem) const {
...
size_t TrackerHit3DCollection::getDatamodelRegistryIndex() const {
  return edm4hep::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT TrackerHit3DCollection::getSchemaVersion() const {
...
namespace {
  ...
  podio::CollectionReadBuffers createBuffers(bool isSubset) {
    ...
    readBuffers.type = "edm4hep::TrackerHit3DCollection";
    readBuffers.schemaVersion = edm4hep::meta::schemaVersion;
    readBuffers.data = isSubset ? nullptr : new TrackerHit3DDataContainer;
    ...
    readBuffers.createCollection = [](const podio::CollectionReadBuffers& buffers, bool isSubsetColl) {
      TrackerHit3DCollectionData data(buffers, isSubsetColl);
      return std::make_unique<TrackerHit3DCollection>(std::move(data), isSubsetColl);
    };

    readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
      // We only have any of these buffers if this is not a subset collection
      if (buffers.data) {
        buffers.data = podio::CollectionWriteBuffers::asVector<edm4hep::TrackerHit3DData>(buffers.data);
      }
    };
    ...
    readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
      if (buffers.data) {
        ...
        // we can delete
        delete static_cast<edm4hep::TrackerHit3DDataContainer*>(buffers.data);
      }
  ...
  bool registerCollection() {
    const static auto reg = []() {
      ...
      factory.registerCreationFunc("edm4hep::TrackerHit3DCollection", edm4hep::meta::schemaVersion, createBuffers);
      ...
      for (unsigned int schemaVersion = 1; schemaVersion < edm4hep::meta::schemaVersion + 1; ++schemaVersion) {
        podio::SchemaEvolution::mutInstance().registerEvolutionFunc(
            "edm4hep::TrackerHit3DCollection", schemaVersion, edm4hep::meta::schemaVersion,
...
TrackerHit3D TrackerHit3DCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<TrackerHit3DObj>((*m_collection)[m_index]);
  return m_object;
}
...
TrackerHit3DCollectionIterator& TrackerHit3DCollectionIterator::operator++() {
  ++m_index;
  return *this;
}

MutableTrackerHit3D TrackerHit3DMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<TrackerHit3DObj>((*m_collection)[m_index]);
  return m_object;
}
...
TrackerHit3DMutableCollectionIterator& TrackerHit3DMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
std::ostream& operator<<(std::ostream& o, const TrackerHit3DCollection& v) {
  ...
  o << "          id:      cellID:        type:     quality:        time:        eDep:   eDepError:                 "
       "position [ x, y, z]:covMatrix [ values]:"
    << '\n';
  ...
  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " " << std::setw(12) << el.getCellID() << " "
      ...
      << std::setw(12) << el.getPosition() << " " << std::setw(12) << el.getCovMatrix() << " " << std::endl;
  }
...
void TrackerHit3DCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
