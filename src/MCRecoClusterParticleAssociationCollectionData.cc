...
MCRecoClusterParticleAssociationCollectionData::MCRecoClusterParticleAssociationCollectionData() :
    m_rel_rec(new std::vector<edm4hep::Cluster>()),
    m_rel_sim(new std::vector<edm4hep::MCParticle>()),
    m_data(new MCRecoClusterParticleAssociationDataContainer()) {
...
MCRecoClusterParticleAssociationCollectionData::MCRecoClusterParticleAssociationCollectionData(
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::MCRecoClusterParticleAssociationData>());
  }
...
void MCRecoClusterParticleAssociationCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoClusterParticleAssociationCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void MCRecoClusterParticleAssociationCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void MCRecoClusterParticleAssociationCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new MCRecoClusterParticleAssociationObj({index, collectionID}, data);
...
bool MCRecoClusterParticleAssociationCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                                   bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::MCRecoClusterParticleAssociationObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::MCRecoClusterParticleAssociationCollection*>(coll);
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      ...
      entries[i]->m_rec = new edm4hep::Cluster((*tmp_coll)[id.index]);
    } else {
      entries[i]->m_rec = nullptr;
    }
  }
...
void MCRecoClusterParticleAssociationCollectionData::makeSubsetCollection() {
