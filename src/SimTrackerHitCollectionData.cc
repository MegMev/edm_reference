...
SimTrackerHitCollectionData::SimTrackerHitCollectionData() :
    m_rel_particle(new std::vector<edm4hep::MCParticle>()), m_data(new SimTrackerHitDataContainer()) {
  m_refCollections.emplace_back(std::make_unique<std::vector<podio::ObjectID>>());
}

SimTrackerHitCollectionData::SimTrackerHitCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::SimTrackerHitData>());
  }
...
void SimTrackerHitCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers SimTrackerHitCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void SimTrackerHitCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void SimTrackerHitCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new SimTrackerHitObj({index, collectionID}, data);
...
bool SimTrackerHitCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::SimTrackerHitObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::SimTrackerHitCollection*>(coll);
...
void SimTrackerHitCollectionData::makeSubsetCollection() {
