...
MCRecoTrackParticleAssociationCollectionData::MCRecoTrackParticleAssociationCollectionData() :
    m_rel_rec(new std::vector<edm4hep::Track>()),
    m_rel_sim(new std::vector<edm4hep::MCParticle>()),
    m_data(new MCRecoTrackParticleAssociationDataContainer()) {
...
MCRecoTrackParticleAssociationCollectionData::MCRecoTrackParticleAssociationCollectionData(
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::MCRecoTrackParticleAssociationData>());
  }
...
void MCRecoTrackParticleAssociationCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoTrackParticleAssociationCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void MCRecoTrackParticleAssociationCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void MCRecoTrackParticleAssociationCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new MCRecoTrackParticleAssociationObj({index, collectionID}, data);
...
bool MCRecoTrackParticleAssociationCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                                 bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::MCRecoTrackParticleAssociationObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::MCRecoTrackParticleAssociationCollection*>(coll);
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      ...
      entries[i]->m_rec = new edm4hep::Track((*tmp_coll)[id.index]);
    } else {
      entries[i]->m_rec = nullptr;
    }
  }
...
void MCRecoTrackParticleAssociationCollectionData::makeSubsetCollection() {
