...
TrackerHit3DCollectionData::TrackerHit3DCollectionData() : m_data(new TrackerHit3DDataContainer()) {
}

TrackerHit3DCollectionData::TrackerHit3DCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::TrackerHit3DData>());
  }
...
void TrackerHit3DCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers TrackerHit3DCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void TrackerHit3DCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void TrackerHit3DCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new TrackerHit3DObj({index, collectionID}, data);
...
bool TrackerHit3DCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                               bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::TrackerHit3DObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::TrackerHit3DCollection*>(coll);
...
void TrackerHit3DCollectionData::makeSubsetCollection() {
