...
RecIonizationClusterCollectionData::RecIonizationClusterCollectionData() :
    m_rel_trackerPulse(new std::vector<edm4hep::TrackerPulse>()), m_data(new RecIonizationClusterDataContainer()) {
  m_refCollections.emplace_back(std::make_unique<std::vector<podio::ObjectID>>());
}

RecIonizationClusterCollectionData::RecIonizationClusterCollectionData(podio::CollectionReadBuffers buffers,
                                                                       bool isSubsetColl) :
    m_rel_trackerPulse(new std::vector<edm4hep::TrackerPulse>()),
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::RecIonizationClusterData>());
  }
...
void RecIonizationClusterCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
  ...
  // clear relations to trackerPulse. Make sure to unlink() the reference data as they may be gone already.
  for (auto& pointer : m_rel_trackerPulse_tmp) {
    for (auto& item : *pointer) {
      item.unlink();
    }
  }
  m_rel_trackerPulse_tmp.clear();
  if (m_rel_trackerPulse) {
    for (auto& item : (*m_rel_trackerPulse)) {
      item.unlink();
    }
    m_rel_trackerPulse->clear();
  }
...
podio::CollectionWriteBuffers RecIonizationClusterCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void RecIonizationClusterCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
  ...
  int trackerPulse_index = 0;
  for (size_t i = 0, size = m_data->size(); i != size; ++i) {
    (*m_data)[i].trackerPulse_begin = trackerPulse_index;
    (*m_data)[i].trackerPulse_end += trackerPulse_index;
    trackerPulse_index = (*m_data)[i].trackerPulse_end;
    for (const auto& it : (*m_rel_trackerPulse_tmp[i])) {
...
void RecIonizationClusterCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new RecIonizationClusterObj({index, collectionID}, data);

    obj->m_trackerPulse = m_rel_trackerPulse.get();
...
void RecIonizationClusterCollectionData::createRelations(RecIonizationClusterObj* obj) {
  // We take ownership of these here
  m_rel_trackerPulse_tmp.emplace_back(obj->m_trackerPulse);
}

bool RecIonizationClusterCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                       bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::RecIonizationClusterObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::RecIonizationClusterCollection*>(coll);
  ...
  for (unsigned int i = 0, size = m_refCollections[0]->size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      podio::CollectionBase* coll = nullptr;
      if (!collectionProvider->get(id.collectionID, coll)) {
        m_rel_trackerPulse->emplace_back(edm4hep::TrackerPulse::makeEmpty());
        continue;
      }
      edm4hep::TrackerPulseCollection* tmp_coll = static_cast<edm4hep::TrackerPulseCollection*>(coll);
      const auto tmp = (*tmp_coll)[id.index];
      m_rel_trackerPulse->emplace_back(tmp);
    } else {
      m_rel_trackerPulse->emplace_back(edm4hep::TrackerPulse::makeEmpty());
    }
  }
...
void RecIonizationClusterCollectionData::makeSubsetCollection() {
  ...
  m_rel_trackerPulse.reset(nullptr);
