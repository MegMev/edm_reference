...
SimCalorimeterHitCollection::SimCalorimeterHitCollection(SimCalorimeterHitCollectionData&& data, bool isSubsetColl) :
    m_isValid(false),
    m_isPrepared(false),
...
SimCalorimeterHit SimCalorimeterHitCollection::operator[](std::size_t index) const {
  return SimCalorimeterHit(m_storage.entries[index]);
}

SimCalorimeterHit SimCalorimeterHitCollection::at(std::size_t index) const {
  return SimCalorimeterHit(m_storage.entries.at(index));
}

MutableSimCalorimeterHit SimCalorimeterHitCollection::operator[](std::size_t index) {
  return MutableSimCalorimeterHit(podio::utils::MaybeSharedPtr(m_storage.entries[index]));
}

MutableSimCalorimeterHit SimCalorimeterHitCollection::at(std::size_t index) {
  return MutableSimCalorimeterHit(podio::utils::MaybeSharedPtr(m_storage.entries.at(index)));
}

std::size_t SimCalorimeterHitCollection::size() const {
  return m_storage.entries.size();
}

std::size_t SimCalorimeterHitCollection::max_size() const {
  return m_storage.entries.max_size();
}

bool SimCalorimeterHitCollection::empty() const {
  return m_storage.entries.empty();
}

void SimCalorimeterHitCollection::setSubsetCollection(bool setSubset) {
...
MutableSimCalorimeterHit SimCalorimeterHitCollection::create() {
  if (m_isSubsetColl) {
    throw std::logic_error("Cannot create new elements on a subset collection");
  }

  auto obj = m_storage.entries.emplace_back(new SimCalorimeterHitObj());
...
void SimCalorimeterHitCollection::prepareForWrite() const {
...
void SimCalorimeterHitCollection::push_back(const MutableSimCalorimeterHit& object) {
  ...
  if (!m_isSubsetColl) {
    ...
    push_back(SimCalorimeterHit(object));
  }
}

void SimCalorimeterHitCollection::push_back(const SimCalorimeterHit& object) {
  if (!m_isSubsetColl) {
...
podio::CollectionWriteBuffers SimCalorimeterHitCollection::getBuffers() {
  return m_storage.getCollectionBuffers(m_isSubsetColl);
}

std::vector<std::uint64_t> SimCalorimeterHitCollection::cellID(const size_t nElem) const {
...
std::vector<float> SimCalorimeterHitCollection::energy(const size_t nElem) const {
...
std::vector<edm4hep::Vector3f> SimCalorimeterHitCollection::position(const size_t nElem) const {
...
size_t SimCalorimeterHitCollection::getDatamodelRegistryIndex() const {
  return edm4hep::meta::DatamodelRegistryIndex::value();
}

podio::SchemaVersionT SimCalorimeterHitCollection::getSchemaVersion() const {
...
namespace {
  ...
  podio::CollectionReadBuffers createBuffers(bool isSubset) {
    ...
    readBuffers.type = "edm4hep::SimCalorimeterHitCollection";
    readBuffers.schemaVersion = edm4hep::meta::schemaVersion;
    readBuffers.data = isSubset ? nullptr : new SimCalorimeterHitDataContainer;
    ...
    readBuffers.createCollection = [](const podio::CollectionReadBuffers& buffers, bool isSubsetColl) {
      SimCalorimeterHitCollectionData data(buffers, isSubsetColl);
      return std::make_unique<SimCalorimeterHitCollection>(std::move(data), isSubsetColl);
    };

    readBuffers.recast = [](podio::CollectionReadBuffers& buffers) {
      // We only have any of these buffers if this is not a subset collection
      if (buffers.data) {
        buffers.data = podio::CollectionWriteBuffers::asVector<edm4hep::SimCalorimeterHitData>(buffers.data);
      }
    };
    ...
    readBuffers.deleteBuffers = [](podio::CollectionReadBuffers& buffers) {
      if (buffers.data) {
        ...
        // we can delete
        delete static_cast<edm4hep::SimCalorimeterHitDataContainer*>(buffers.data);
      }
  ...
  bool registerCollection() {
    const static auto reg = []() {
      ...
      factory.registerCreationFunc("edm4hep::SimCalorimeterHitCollection", edm4hep::meta::schemaVersion, createBuffers);
      ...
      for (unsigned int schemaVersion = 1; schemaVersion < edm4hep::meta::schemaVersion + 1; ++schemaVersion) {
        podio::SchemaEvolution::mutInstance().registerEvolutionFunc(
            "edm4hep::SimCalorimeterHitCollection", schemaVersion, edm4hep::meta::schemaVersion,
...
SimCalorimeterHit SimCalorimeterHitCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<SimCalorimeterHitObj>((*m_collection)[m_index]);
  return m_object;
}
...
MutableSimCalorimeterHit SimCalorimeterHitMutableCollectionIterator::operator*() {
  m_object.m_obj = podio::utils::MaybeSharedPtr<SimCalorimeterHitObj>((*m_collection)[m_index]);
  return m_object;
}
...
SimCalorimeterHitMutableCollectionIterator& SimCalorimeterHitMutableCollectionIterator::operator++() {
  ++m_index;
  return *this;
}
...
std::ostream& operator<<(std::ostream& o, const SimCalorimeterHitCollection& v) {
  ...
  for (const auto&& el : v) {
    o << std::scientific << std::showpos << std::setw(12) << el.id() << " " << std::setw(12) << el.getCellID() << " "
      << std::setw(12) << el.getEnergy() << " " << std::setw(12) << el.getPosition() << " " << std::endl;

    o << "      contributions : ";
    for (unsigned j = 0, N = el.contributions_size(); j < N; ++j) {
      o << el.getContributions(j).id() << " ";
    }
    o << std::endl;
  }
...
void SimCalorimeterHitCollection::print(std::ostream& os, bool flush) const {
  os << *this;
  if (flush) {
    os.flush();
  }
