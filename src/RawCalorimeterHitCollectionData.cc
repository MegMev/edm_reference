...
RawCalorimeterHitCollectionData::RawCalorimeterHitCollectionData() : m_data(new RawCalorimeterHitDataContainer()) {
}

RawCalorimeterHitCollectionData::RawCalorimeterHitCollectionData(podio::CollectionReadBuffers buffers,
                                                                 bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::RawCalorimeterHitData>());
  }
...
void RawCalorimeterHitCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers RawCalorimeterHitCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void RawCalorimeterHitCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void RawCalorimeterHitCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new RawCalorimeterHitObj({index, collectionID}, data);
...
bool RawCalorimeterHitCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                    bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::RawCalorimeterHitObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::RawCalorimeterHitCollection*>(coll);
...
void RawCalorimeterHitCollectionData::makeSubsetCollection() {
