...
MCRecoTrackerAssociationCollectionData::MCRecoTrackerAssociationCollectionData() :
    m_rel_rec(new std::vector<edm4hep::TrackerHit>()),
...
MCRecoTrackerAssociationCollectionData::MCRecoTrackerAssociationCollectionData(podio::CollectionReadBuffers buffers,
                                                                               bool isSubsetColl) :
    m_rel_rec(new std::vector<edm4hep::TrackerHit>()),
    m_rel_sim(new std::vector<edm4hep::SimTrackerHit>()),
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::MCRecoTrackerAssociationData>());
  }
...
void MCRecoTrackerAssociationCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers MCRecoTrackerAssociationCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void MCRecoTrackerAssociationCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void MCRecoTrackerAssociationCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new MCRecoTrackerAssociationObj({index, collectionID}, data);
...
bool MCRecoTrackerAssociationCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                           bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::MCRecoTrackerAssociationObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::MCRecoTrackerAssociationCollection*>(coll);
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      ...
      if (auto TrackerHit3DColl = dynamic_cast<edm4hep::TrackerHit3DCollection*>(coll)) {
        entries[i]->m_rec = new edm4hep::TrackerHit((*TrackerHit3DColl)[id.index]);
      } else if (auto TrackerHitPlaneColl = dynamic_cast<edm4hep::TrackerHitPlaneCollection*>(coll)) {
        entries[i]->m_rec = new edm4hep::TrackerHit((*TrackerHitPlaneColl)[id.index]);
      }
    } else {
      entries[i]->m_rec = nullptr;
    }
...
void MCRecoTrackerAssociationCollectionData::makeSubsetCollection() {
