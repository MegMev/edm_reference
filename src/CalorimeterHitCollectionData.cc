...
CalorimeterHitCollectionData::CalorimeterHitCollectionData() : m_data(new CalorimeterHitDataContainer()) {
}

CalorimeterHitCollectionData::CalorimeterHitCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::CalorimeterHitData>());
  }
...
podio::CollectionWriteBuffers CalorimeterHitCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void CalorimeterHitCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
...
bool CalorimeterHitCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider,
                                                 bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::CalorimeterHitObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::CalorimeterHitCollection*>(coll);
