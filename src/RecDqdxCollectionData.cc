...
RecDqdxCollectionData::RecDqdxCollectionData() :
    m_rel_track(new std::vector<edm4hep::Track>()), m_data(new RecDqdxDataContainer()) {
  m_refCollections.emplace_back(std::make_unique<std::vector<podio::ObjectID>>());
}

RecDqdxCollectionData::RecDqdxCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
    m_rel_track(new std::vector<edm4hep::Track>()),
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<edm4hep::RecDqdxData>());
  }
...
void RecDqdxCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers RecDqdxCollectionData::getCollectionBuffers(bool isSubsetColl) {
...
void RecDqdxCollectionData::prepareForWrite(bool isSubsetColl) {
  for (auto& pointer : m_refCollections) {
    pointer->clear();
  }
...
void RecDqdxCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (auto& data : *m_data) {
    auto obj = new RecDqdxObj({index, collectionID}, data);
...
bool RecDqdxCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      edm4hep::RecDqdxObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<edm4hep::RecDqdxCollection*>(coll);
  ...
  for (unsigned int i = 0, size = entries.size(); i != size; ++i) {
    ...
    if (id.index != podio::ObjectID::invalid) {
      podio::CollectionBase* coll = nullptr;
      if (!collectionProvider->get(id.collectionID, coll)) {
        entries[i]->m_track = nullptr;
        continue;
      }
      edm4hep::TrackCollection* tmp_coll = static_cast<edm4hep::TrackCollection*>(coll);
      entries[i]->m_track = new edm4hep::Track((*tmp_coll)[id.index]);
    } else {
      entries[i]->m_track = nullptr;
    }
  }
...
void RecDqdxCollectionData::makeSubsetCollection() {
